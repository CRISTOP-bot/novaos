...
extern void isr0(void);extern void isr6(void);extern void isr8(void);extern void isr13(void);extern void isr14(void);extern void isr_default(void);
static void set_gate(int n,void(*fn)(void)){uint64_t x=(uint64_t)fn;idt[n]=(struct idt_gate){x,8,0,0x8e,x>>16,x>>32,0};}
void idt_init(void){for(int i=0;i<256;i++)set_gate(i,isr_default); set_gate(0,isr0);set_gate(6,isr6);set_gate(8,isr8);set_gate(13,isr13);set_gate(14,isr14);idtr.limit=sizeof(idt)-1;idtr.base=(uint64_t)idt;__asm__ volatile("lidt %0"::"m"(idtr));}
