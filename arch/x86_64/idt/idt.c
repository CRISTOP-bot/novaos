...
extern void ring3_return_point(void); extern bool syscall_exit_seen(void); extern uint64_t ring3_resume_stack; extern bool returned_from_ring3;
void ring3_return_dispatch(struct frame *f){ if((f->cs&3)==3 && syscall_exit_seen()){ f->rip=(uint64_t)(uintptr_t)ring3_return_point; f->cs=0x08; f->rflags=(f->rflags & ~(3ULL << 12)) | 0x202; f->rsp=ring3_resume_stack; f->ss=0x10; returned_from_ring3=true; } }
