...
bool memory_diagnostics_self_test(void) {
    ...
    if (paging_map_count() != map_before + 1 || !paging_translate(0xffff900200000000ULL, &translated) || translated != physical) return false;
    used_after_map = pmm_used_pages();
    if (!paging_unmap_page(0xffff900200000000ULL) || paging_unmap_count() != unmap_before + 1 || paging_translate(0xffff900200000000ULL, &translated)) return false;
    pmm_free_page(page); if (pmm_free_count() != before.pmm_frees + 1 || pmm_used_pages() + 1 != used_after_map) return false;
    active_before = before.heap_active_allocations; a=kmalloc(24); b=kmalloc(128); c=kmalloc(2048); if (!a||!b||!c) return false;
