...
for i in $(seq 1 100); do
  if [ -f "$log" ] && grep -q 'NOVAOS_BOOT_OK' "$log" && grep -q 'NOVAOS_PMM_OK' "$log" && grep -q 'NOVAOS_PAGING_OK' "$log" && grep -q 'NOVAOS_HEAP_OK' "$log" && grep -q 'NOVAOS_MEMORY_OK' "$log" && grep -q 'NOVAOS_RING3_OK' "$log" && grep -q 'NOVAOS_SCHEDULER_OK' "$log" && grep -q 'NOVAOS_PROCESS_OK' "$log" && grep -q 'NOVAOS_SYSCALL_OK' "$log" && grep -q 'NOVAOS_VFS_OK' "$log"; then
